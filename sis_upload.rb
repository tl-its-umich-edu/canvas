	...
	if (Dir[securityFile].length != 1)
		...
		File.open(securityFile, 'r') do |sFile|
			while line = sFile.gets
				...
				if (env_array.size != 3)
					return "security file should have the settings in format of: token=TOKEN,server=SERVER,directory=DIRECTORY"
				end
				token_array=env_array[0].split('=')
				$token=token_array[1]
				server_array=env_array[1].split('=')
				$server=server_array[1]
				directory_array=env_array[2].split('=')
	...
	if (Dir[propertiesFile].length != 1)
		...
		File.open(propertiesFile, 'r') do |pFile|
			while line = pFile.gets
				...
				env_array = line.strip.split(',')
				if (env_array.size != 1)
					return "properties file should have the settings in format of: sleep=SLEEP"
				end
				sleep_array=env_array[0].split('=')
...
# the interval in seconds between API calls: curl '#{server}/api/v1/accounts/1/sis_imports/#{process_id}
$sleep = 10
